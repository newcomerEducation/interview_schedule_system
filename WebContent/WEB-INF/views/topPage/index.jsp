			...
			<tbody>
				...
				<c:forEach var="schedule" items="${schedules}" varStatus="status">
				 ...
				 <td class="schedule_sales_employee_name"><c:out
								value="${schedule.sales_employee.employee_name}" />
						...
						<td class="schedule_aspiration_situation"><%-- <c:out
								value="${schedule.aspiration_situation}" />--%>
						<c:choose>
							<c:when test="${schedule.aspiration_situation == 1}">確認中</c:when>
							<c:when test="${schedule.aspiration_situation == 2}">志望する</c:when>
							<c:when test="${schedule.aspiration_situation == 3}">志望しない</c:when>
							<c:otherwise>（未入力）</c:otherwise>
						</c:choose></td>

						<td class="schedule_interview_situation"><%-- <c:out
								value="${schedule.interview_situation}" />--%>
						<c:choose>
						...
						Schedule schedule = (Schedule)request.getAttribute("shedules");
						LocalDateTime time = schedule.getFirst_interview_scheduled_time();
						String datetime = datetimeformatter.format(time);--%>

					<%--//	String time = '${schedule.first_interview_scheduled_time}'.toString()   --%>
						<c:out
								value="${datetime}" /></td>
